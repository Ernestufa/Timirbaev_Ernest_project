...
std::vector<int> Graph::Metod_Deikstra(int StartNode, int EndNode) const
{
	if (edges.empty()) {
...
double Graph::Lenght_ShortestPath(vector<int>& path)
{
	double length = 0.0;
...
bool Graph::BFS_MaxFlow(vector<vector<double>>& capacity, int StartNode, int EndNode, vector<int>& parent) const
{
	vector<bool> visited(SizeGraph, false);
	queue<int> q;
	q.push(StartNode);
